...
InvoiceItems.before.update(function (userId, doc, fieldNames, modifier, options) {
    ...
    var quantity = modifier.$set.quantity || doc.quantity;
    var price = modifier.$set.price || doc.price;
    modifier.$set.amount = quantity * price;
});
...
InvoiceItems.after.insert(function (userId, doc) {
    ...
    var sum = 0;
    InvoiceItems.find({invoiceId: doc.invoiceId}).map(function (item) {
        sum += item.amount;
    });
